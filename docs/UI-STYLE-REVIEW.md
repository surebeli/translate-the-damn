...
> **⚠️ 后续结论（2026-06-19，本评审之后）**：基于本文档，macOS 已**定型为单一「清晰风」UI**——
> 单页原生 grouped `Form` 设置 + 斜体原文玻璃卡弹窗（即下文的 DS 风），并**移除了其余 6 种风格与 `uiStyle`
> 切换机制**（见 `PARITY.md`，合入 main 的 merge `1e39771`）。首次运行默认弹窗视觉为 **acrylic**（solid 仍可在
> 设置里切换）。**下文对 7 种风格的评审属历史决策依据**，截图与 dev 截图 harness 来自评审分支 `expe/202606`，
> 不在 main 上。
>
