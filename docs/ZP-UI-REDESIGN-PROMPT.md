...
>
> 这是一个**可复用模板**：ZP 与 O48 都是用它落地的。要再做一个新风格时，先读《设计自由 vs 设计门限》一节——视觉身份由你发挥，但门限必须全部满足。
...
目前共有三种风格：**Classic**（最早实现）、**ZP**、**O48**（当前默认）。切换通过 `config.general.uiStyle`（`"classic"` / `"ZP"` / `"O48"`，nil → 当前默认 `"O48"`）+ **每个设置页各自的 Picker** 实现；选择逻辑是"多路 switch + default 兜底"。切换后关闭再打开设置窗即生效（`openSettings` 每次按磁盘 config 新建 controller），浮窗则在保存时经 `hotReload` 立即重建。新风格作为第四种加入，Classic/ZP/O48 全部保留。
...
> 本文件最初为 ZP 写，O48 是用同一套方法落地的第二个新风格。要再做一个新风格时：把上面的《设计原则（ZP 风格）》以及下面《实现规范》里浮窗/设置的**具体设计**都当成*一种示例答案*，不是必须照抄的样式。**视觉身份归你发挥**，但下面的门限不可逾越。
...
- **每个设置页**（`SettingsView` / `ZPSettingsView` / `O48SettingsView`）的 `uiStyle` Picker 都要加上你的新风格选项；保存到 `config.general.uiStyle`。
