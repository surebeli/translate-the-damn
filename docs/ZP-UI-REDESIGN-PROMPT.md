...
>
> 这是一个**可复用模板**：ZP、O48、KM、Z 都是用它落地的。要再做一个新风格时，先读《设计自由 vs 设计门限》一节——视觉身份由你发挥，但门限必须全部满足。
...
目前共有五种风格：**Classic**（最早实现）、**ZP**（磨砂 popover）、**KM**（侧栏 + 紧凑浮窗）、**O48**（聚焦 HUD，当前默认）、**Z**（文档卡，最新）。切换通过 `config.general.uiStyle`（`"classic"` / `"ZP"` / `"km"` / `"O48"` / `"Z"`，nil → 当前默认 `"O48"`）+ **每个设置页各自的 Picker** 实现；选择逻辑是"多路 switch + default 兜底"。切换后关闭再打开设置窗即生效（`openSettings` 每次按磁盘 config 新建 controller），浮窗则在保存时经 `hotReload` 立即重建。新风格作为第六种加入，Classic/ZP/KM/O48/Z 全部保留。
...
- **每个设置页**（`SettingsView` / `ZPSettingsView` / `KMSettingsView` / `O48SettingsView` / `ZSettingsView`）的 `uiStyle` Picker 都要加上你的新风格选项；保存到 `config.general.uiStyle`。
...
├── Section "通用": Picker(uiStyle, .segmented: Z/KM/ZP/O48/Classic) + Toggle(startWithWindows)
