...
> 不切窗口、不跳应用,尽量不打断你的阅读,而且**专业术语原样保留**(`OAuth`、`API`、`GDPR` 照旧)。它**复用你已经在用的
> 大模型**——API、订阅、命令行 CLI 都行,自带 Key、成本极低,开源免费。
...
- **🔌 复用你已有的大模型访问方式 —— 不绑死任何一家。** 同一个声明式清单(`spec/backends.json`)后面挂着**三类**后端,各有取舍:
  ...
  2. **HTTP(复用订阅 / 复用 API)**:经 OpenAI / Anthropic 协议把你**已有的大模型访问**接进来——**复用订阅**(有些大模型订阅
     的 token 本是给 agent / coding 用的,这里也能复用,如 Kimi Code、MiMo token-plan)或**复用 API**(已购买的大模型 API 直接拿来用,
     如 DeepSeek),也可指向**任意**兼容端点(自定义服务商,用完即删)。接入最轻、**复用**你已有的额度;但它们是**通用大模型(非专业翻译模型)**,翻译会**慢一点**、偶尔不如专业翻译稳。
  3. **本地 CLI(复用订阅)**:`claude`、`codex`、`copilot`、`agy`(回退 `gemini`)、`opencode`、`kimi`、`mimo`。
     **复用**你已登录的订阅,而且能**用上更多、更强的模型**;代价是每次要冷启动一个 Agent 进程,**最慢**。
- **💸 成本极低。** 走 CLI / HTTP 基本是**复用**你**已经付费**的订阅或 API;走专业翻译 API 也只是几分钱一次。
...
- **🎯 专业术语原样保留,译文拿来就能用。** 英文原文里的技术 / 专业名词 —— `OAuth`、`API`、`atorvastatin`、`GDPR` …… ——
  **保持英文**,只翻译其间的说明性文字;代码块、命令、配置也原样不动。读代码、读论文、读合同时,译文**不会把你认识的关键词
  译成看不懂的中文** —— 这是为**专业阅读**调过的翻译,不是逐字直译,覆盖计算机、医学、法律等场景。
...
| **② HTTP(复用订阅 / 复用 API)** | Kimi / MiMo / DeepSeek 预设 + 自定义 | **~1–5 秒**(实测) | 复用已有额度、接入最轻;**通用大模型(非专业翻译)**,故略慢 |
| **③ 本地 CLI(复用订阅)** | `codex`/`kimi`/`opencode`/`mimo` ~5–8s;`claude`/`copilot` ~10–16s | **~5–16 秒**(实测) | 能用上**更多更强的模型**,代价是每次冷启动一个 Agent 进程 |

一句话:**要又快又稳的翻译 → 专业翻译 API ①**;**要复用已有订阅 / API、接入最轻 → HTTP ②**(通用模型,稍慢);**要复用订阅又想用更强模型
...
- **本地 CLI** 需已安装且**已登录**;它们较重(推理 + 上下文),一次翻译约 5–16 秒,换来「复用订阅 + 更强模型」——**想又快又稳就用专业翻译 API**。CLI 从中性沙箱
