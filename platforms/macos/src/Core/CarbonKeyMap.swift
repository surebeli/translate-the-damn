...
public enum CarbonKeyMap {
    ...
    public static func carbonModifiers(hasControl: Bool, hasAlt: Bool, hasShift: Bool, hasWin: Bool) -> UInt32 {
        var mods: UInt32 = 0
        // macOS porting convention: Windows Ctrl → macOS ⌘ Command (0x0100); Alt → ⌥ Option;
        // Shift → ⇧. This makes the macOS default "Ctrl+Shift+C" register as ⇧⌘C
        // (Shift+Command+C) — the Mac-appropriate hotkey — while the config/display string stays
        // "Ctrl+Shift+C" (shared Windows-naming schema, Law 5). (Win maps to ⌘ as well, below.)
